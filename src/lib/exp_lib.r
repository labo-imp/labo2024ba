...
exp_wf_init <- function( pnombrewf )
{
  ...
  output$miAmbiente <- envg$EXPENV$miAmbiente
  output$messenger <- envg$EXPENV$messenger
  output$scriptname <- envg$EXPENV$scriptname
  

  # traigo la semilla
  cat( "por abrir ambiente\n")
  arch_ambiente <- paste0( envg$EXPENV$bucket_dir, "/", envg$EXPENV$arch_ambiente )
  miAmbiente <- read_yaml( arch_ambiente )
  ksemilla_azar <- miAmbiente$semilla_primigenia
  output$semilla <- ksemilla_azar
  cat( "ambiente abierto\n")
...
exp_init <- function( pbypass=FALSE )
{
  ...
  output$expenv$miAmbiente <- env_up2$param_local$miAmbiente
