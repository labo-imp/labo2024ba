...
cat( "z2502_EV_evaluate_conclase_gan_semillio.r  START\n")
action_inicializar() 
...
for ( irank in ranks ) {
  ...
  repeticiones <- tb_predicciones[ rank==irank, unique(repeticion) ]

  for( repe in repeticiones )
  {
    gan_sum <- paste0( "gan_sum_", irank )
    gan_semillerio <- paste0( "gan_semi_", irank )
    tb_ganancias[ , paste0(gan_sum) := 0 ]

    isems <- tb_predicciones[ rank==irank & repeticion==repe & isem > 0, unique( isem )]
    ...
    for (vsem in isems)
    {
      ...
      campito <- tb_predicciones[ rank==irank & repeticion==repe & isem==vsem, campo]
      ...
      linea <- list()
      linea$rank <- irank
      linea$iteracion_bayesiana <- tb_predicciones[ rank==irank & repeticion==repe & isem==vsem, iteracion_bayesiana]
      linea$qsemillas <- 1
      linea$semilla <- tb_predicciones[ rank==irank & repeticion==repe & isem==vsem, semilla]
    ...
    # promedio de las ganancias ( atencion que NO es semillerio )
    tb_ganancias[, paste0(gan_sum) := get(gan_sum) / length(isems) ]

    # calculo la ganancia de la precalculada prediccion del semillerio
    campito <- tb_predicciones[ rank==irank & repeticion==repe & semillerio==1, campo]
    temp_pred <- tb_future_prediccion[ , c( campito, envg$PARAM$dataset_metadata$clase ), with=FALSE ]
    setorderv( temp_pred, campito, -1 )
    temp_pred[, ganancia_acum :=
        cumsum(ifelse(get(envg$PARAM$dataset_metadata$clase) %in% envg$PARAM$train$positivos, envg$PARAM$train$gan1, envg$PARAM$train$gan0))]

   tb_ganancias[, paste0(gan_semillerio) := temp_pred$ganancia_acum ]

    # calculo la mayor ganancia  SUAVIZADA
    tb_ganancias[, gan_suavizada := frollmean(
      x = get(gan_semillerio),
      n = envg$PARAM$graficar$ventana_suavizado,
      align = "center",
      na.rm = TRUE,
      hasNA = TRUE
    ...
    linea <- list()
    linea$rank <- irank
    linea$iteracion_bayesiana <- tb_predicciones[ rank==irank & repeticion==repe, min(iteracion_bayesiana) ]
    ...
    plot(
      ...
      y = tb_ganancias[envios >= envg$PARAM$graficar$envios_desde, get(tb_predicciones[ rank==irank & repeticion==repe & isem==1, campo]) ],
      type = "l",
      col = "gray",
      xlim = c(envg$PARAM$graficar$envios_desde, envg$PARAM$graficar$envios_hasta),
      ylim = c(ymin, ymax),
      main = paste0("repeticion= ", repe, ", mejor gan prom = ", as.integer(ganancia_suavizada_max)),
      xlab = "Envios",
      ylab = "Ganancia",
      panel.first = grid()
    )
    ...
    if ( length(isems) > 1) {
      for (s in 2:length(isems))
      {
        lines(
          x = tb_ganancias[envios >= envg$PARAM$graficar$envios_desde, envios],
          y = tb_ganancias[envios >= envg$PARAM$graficar$envios_desde, get(tb_predicciones[ rank==irank & repeticion==repe & isem==s, campo])],
          col = "gray"
         )
      }
    }
    ...
    lines(
      x = tb_ganancias[envios >= envg$PARAM$graficar$envios_desde, envios],
      y = tb_ganancias[envios >= envg$PARAM$graficar$envios_desde, get(gan_semillerio) ],
      col = "blue"
    )
  ...
  arch_ganancias <- paste0(
    ...
    sprintf("%02d", irank),
    "_",
    sprintf("%03d", tb_predicciones[ rank==irank & repeticion==repe, min( iteracion_bayesiana )]),
    ".txt"
   )
...
cat( "z2502_EV_evaluate_conclase_gan_semillerio.r  END\n")
